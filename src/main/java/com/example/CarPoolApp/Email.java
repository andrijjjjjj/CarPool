...
//	}
	...
	public void emailAcceptedRide(User user) {
		this.user = user;
		...
		try {
			...
			message.setSubject("Ride Cancellation: ISU Carpool");
	         ...
	         String htmlText = 
	        		  "<img src=\"cid:image\">"
	         		+ "<H1>Hello "+user.getProfile().getfName()+",</H1>"
	         		+ "<H2>Your carpool ride request has been accepted: </H2>"
	         		+ "<p>Tuesday, November 26 2019 @ 5:00 PM</p>"
	         		+ "<H3>Thanks for using ISU Carpool!</H3>"
	         		+ "<H3>http://illinoisstate.edu</H3>";
	         messageBodyPart.setContent(htmlText, "text/html");
			...
			System.out.println("Carpool accepted for ride email sent to: \""+user.getUserID()+"@ilstu.edu"+"\" sucessfully.");
